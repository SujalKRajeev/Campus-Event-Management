...
-- Add composite unique constraint for student_id per college (allowing nulls)
...
-- Add performance indexes (without CONCURRENTLY since we're in a transaction)
CREATE INDEX IF NOT EXISTS events_college_id_idx ON public.events (college_id);
...
CREATE INDEX IF NOT EXISTS analytics_summary_event_id_idx ON public.analytics_summary (event_id);
CREATE INDEX IF NOT EXISTS attendance_registration_id_idx ON public.attendance (registration_id);
CREATE INDEX IF NOT EXISTS feedback_registration_id_idx ON public.feedback (registration_id);